...
<template>
  <div class="card bg-base-200 border border-base-300/60 shadow-xs rounded-m3-lg h-full flex flex-col justify-between overflow-hidden">
    <div class="card-body p-4 sm:p-5 lg:p-5.5 gap-2.5 sm:gap-3 flex-1 flex flex-col justify-between min-h-0">
      ...
      <div class="flex-1 flex flex-col justify-center gap-2.5 my-auto py-1 min-h-0">
        <div class="bg-base-100/90 rounded-m3-lg border border-base-300/50 p-3.5 sm:p-4 lg:p-4.5 flex flex-col gap-2.5 sm:gap-3 shadow-xs">
          <!-- Grille des bornes horaires Arrivée / Départ -->
          <div class="grid grid-cols-2 gap-3 sm:gap-4">
            <!-- Borne Arrivée -->
            <div class="flex flex-col items-center text-center">
              <div class="flex items-center gap-1.5 mb-0.5 text-base-content/70">
                ...
                <span class="text-[11px] sm:text-xs font-semibold uppercase tracking-wider">Arrivée</span>
              </div>
              <span class="text-2xl sm:text-3xl lg:text-4xl font-black text-base-content tracking-tight font-mono my-0.5">
                {{ presence?.check_in_time ? formatTime(presence.check_in_time) : '--:--' }}
              </span>
              <span class="text-[11px] sm:text-xs text-base-content/60 font-medium">
                Prévu à {{ expectedArrivalTime.slice(0, 5) }}
              </span>
            </div>

            <!-- Borne Départ -->
            <div class="flex flex-col items-center text-center border-l border-base-300/80 pl-3 sm:pl-4">
              <div class="flex items-center gap-1.5 mb-0.5 text-base-content/70">
                <div class="w-5 h-5 rounded-full bg-base-200 text-base-content/70 flex items-center justify-center shrink-0">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                ...
                <span class="text-[11px] sm:text-xs font-semibold uppercase tracking-wider">Départ</span>
              </div>
              <span class="text-2xl sm:text-3xl lg:text-4xl font-black text-base-content tracking-tight font-mono my-0.5">
          ...
          <div
            v-if="workDuration"
            class="flex items-center justify-center gap-1.5 text-xs sm:text-sm font-medium py-1.5 px-3 rounded-m3-md bg-base-200/70 border border-base-300/40"
          >
            <svg
          ...
          <div
            v-else
            class="flex items-center justify-center gap-1.5 text-xs text-base-content/65 py-1.5 px-2.5 rounded-m3-md bg-base-200/40 border border-base-300/30 text-center"
          >
